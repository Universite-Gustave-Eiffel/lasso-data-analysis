...
collection, through the prism of sound sources. The analysis only focused on the 
tags checked by the users and not on the sound spectrum of the measurements.

The first objective of the study was to control the quality of the tag data provided by the public, 
either by checking if some known temporal dynamics of sound sources are present in the dataset, or by checking the correspondence 
between some tags related to the acoustic measurement conditions (presence of rain or wind) with the records
from the national meteorological service database.

As these exploratory works will be extended and consolidated thereafter to a more global exploration 
of the database, and with the will to include this study within the framework of the Open Science, an 
...
would be lost each year in Western Europe due to traffic noise [L. Fritschi, L. Brown, R. Kim, D. Schwela, et S. Kephalopolous, « Burden of Disease from Environmental Noise: Quantification of Healthy Life Years Lost in Europe », World Health Organization, 2011.]. Its health, physiological and psychological 
...
his or her living environment, with his or her health-related quality of life or with the restorative effect of a place [D. Botteldooren, L. Dekoninck, et D. Gillis, « The influence of traffic noise on appreciation of the living quality of a neighborhood », International Journal of Environmental Research and Public Health, vol. 8, no 3, p. 777‑798, 2011, doi: 10.3390/ijerph8030777.]. 
The economic and social impact of noise has also been demonstrated with a social cost of noise in France estimated at 
147 billion euros per year [DOUILLET Maia, SIPOS Gala, DELUGIN Léna, BULLIOT Benoît, REMONTET Lucas, BIDAULT Elsa, ADEME, I Care & Consult, BRUITPARIF OBSERVATOIRE DU BRUIT EN ILE-DE-FRANCE, ENERGIES DEMAIN, "Coût social du bruit en France", ADEME, Expertise, 2021]. A complete and precise evaluation of the presence of noise in the environment is 
...
production of data with smartphones in order to eventually propose an alternative method of environmental sound mapping.
...
in particular by the scientific community which would like to develop new tools for the evaluation and representation of sound environments. 
...
Git is already used within the UMRAE laboratory.
...
The first analysis having brought encouraging results, we took it again in order 
...
While these two analyses share a common basis, the selection criteria were refined between the two analyses. 
...
In the exploratory analysis, we decided to keep tracks that are comprised in a rather small area,
the bounding box area would have to be equivalent to a square of 25 by 25 meters. This criterion was not considered in the extended analysis, but a threshold of 8 m/s (equivalent to 30 km/h) was set between 2 GPS measurements to eliminate tracks recorded in motor vehicles.
...
crowdsourced acoustic data and if tags are sufficient to describe user environment.
...
A reduced toolbox also promotes reproducibility by limiting the constraints of 
installing software on a machine.
Nevertheless, the analysis uses a large list of R packages and some of them may 
have dependencies that can cause problems.
