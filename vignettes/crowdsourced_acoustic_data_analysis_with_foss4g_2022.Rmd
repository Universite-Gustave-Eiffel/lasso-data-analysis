...
to analyse and produce knowlege from it.
...
PostGIS database.
The materialized view is a good compromise because it allowed us to recompute a view 
easily if the threshold values changed while having better response times than with a normal view.
...
So we separated overseas records from metropolitan ones using R.

A Rmarkdown document establishes the connection with the materialized view and then performs 
the operations allowing to finalize data preparation and analyze the data.
