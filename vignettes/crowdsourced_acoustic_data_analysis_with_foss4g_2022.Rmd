...
As the NoiseCapture application is Open Source and the data collection infrastructure use Free 
and Open Source Softwares (FOSS), we decided to pursue this by using open source software for data analysis.
...
(1 second). Each trace is timestamped too.
...
The climate and sound dynamics are also well known by the authors and are well documented.
...
The raw data has been stored and pre-filtered in a geospatial databases and has been
analysed using a statistical analysis language.

### Tools

In order to be as open as possible and to work in the most collaborative way, we choose
decided in a FOSS toolset: PostGreSQL/Postgis to store the data, the R language to analyse and produce knowlege from the data. We also used Git as it is a great tool to facilitate remote collaboration.
...
These criteria are both attributive and spatial. We removed from the analysis the tracks taggued *test* or *indoor* as we wanted outdoor recording. 
We also filtered too short tracks (less than 5 seconds) and too long tracks (more than 15 minutes). We kept the tracks where the GPS median accuracy was higher than 20 meters. We also decided to keep tracks that are comprised in a rather small area (les than 625 square meters which correspond to a 25 by 25 meters square).
We also pre-filtered the data using the NaturalEarth dataset to keep record within the France country. 
NaturalEarth is a great dataset and is well known worlwide however it does not
distinguish France metropolitan area from overseas areas. This could have introduce
complications with timezone handling and the seasons are not the same everywhere.
So we had to separate overseas records from metropolitan ones. 
We did that operation using R.

A Rmarkdown document establishes the connection with the view and then performs 
the operations allowing to finalyze data preparation and analyze the data.
It is a document mixing narrative, figures and code (in R but also several programming languages).

In this document, we refine the spatial filtering to France's metropilotan area 
using an authoritative dataset [@institutgeographiquenationalAdminExpress].
We also transform the track timestamp to local time and season using the R packages 
...
We also used the suncalc package (@thieurmelSuncalcComputeSun2019) to compute the sunrise time of the day of the track at its location.

# Results
## Environmental acoustics results 

The main goal of this premliminary study was to verify the scientific value of 
...
In the sample studied, it is possible to note a prevalence of the tags *road*, 
*chatting*, *animals* and *wind*. 
The tags *air_traffic* and *works* are also well represented.

```{r, fig.cap="Tags repartition", include=FALSE}
knitr::include_graphics("plots/tags_repartition.png", dpi = 72)
```
...
```{r, fig.cap="Animals dynamics around sunrise", include=FALSE}
knitr::include_graphics("plots/animal_dynamics_around_sunrise.png", dpi = 72)
```

This is a common dynamic for bird song [@robbins1981effect]. 
We also observed peaks in human activity, especially commuting [@MA2017135].
