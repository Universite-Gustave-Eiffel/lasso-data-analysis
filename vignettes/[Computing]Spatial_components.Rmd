...
df_fr <- all_info_tempo %>% dplyr::filter(admin == "France") %>% distinct(pk_track, .keep_all= TRUE) %>% dplyr::select(c(lon,lat))

dend <- hclust(dist(df_fr), method="complete")
plot(dend, hang=-1)
points(c(-100,100), c(2,2), col="red", type="l", lty=2)
groups <- cutree(df_fr, h=2)
