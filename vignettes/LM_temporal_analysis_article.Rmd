...
author: "Ludovic Moisan from Nicolas Roelandt"
supervisors: "Pierre aumond, Paul Chapron"
date: "`r Sys.Date()`"
...
user="noisecapture",
password= "yvbxGBQqcATg75Yzea" #Sys.getenv('noisecapture_password') # password stored in .Renviron. Use this to edit it : usethis::edit_r_environ()
)
```
...
filtered_track_info <- st_join(filtered_track_info, countries, largest = TRUE)
...
query <- "SELECT pk_track, measure_count FROM noisecapture_track_frame"

measures <- RPostgreSQL::dbGetQuery(con,statement = query)
...
query <- "SELECT  ct.pk_track, record_utc, time_length, pleasantness, noise_level, track_uuid, geog FROM clean_tracks as ct ;"

test <- sf::st_read(con,query = query)

test <- filtered_track_info %>% left_join(tags, by = "pk_track")

test %>% filter(measure_count > 5 )

length(unique(test$pk_track))
...
filtered_track_info <- st_join(filtered_track_info, countries)

filtered_track_info <- left_join(filtered_track_info, measures)
...
`length(unique(track_points\$pk_track))`
...
#Stations list
setwd("~/Stage/Scripts/lasso-data-analysis-main/raw_data")
...
ggplot() +
  geom_col(aes(x = c("-no rain-","(0,0.2]","(0.2,0.4]","(0.4,0.6]","(0.6,1.4]","(1.4,2]","(2,7]","(7,+)"), y = rain_prop)) +
  labs(
